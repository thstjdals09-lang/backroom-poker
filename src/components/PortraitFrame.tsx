...
//   1) public/portraits/<imageKey ?? npcId>.png 가 있으면 그 실제 아트를 쓰고
//   2) 없으면 npcId 기준 픽셀 초상화로 자동 대체하고
//   3) 그것도 없으면 이니셜 실루엣을 보여준다.
// imageKey는 같은 인물의 표정 변형(taesik_annoyed 등)을 가리킬 수 있다.
...
export default function PortraitFrame({
  ...
  const srcKey = imageKey ?? npcId;
  const frame = (
    <div className={`portrait-frame ${size === 'sm' ? 'portrait-frame--sm' : ''}`}>
      {illustrated && npcId ? (
        <ImageWithFallback
          key={srcKey}
          src={`${import.meta.env.BASE_URL}portraits/${srcKey}.png`}
          alt={name}
